#!/bin/bash
# Targeted scheduler for the 32 incomplete evaluations (resume):
#   - Locomotion: clusters 0-19  (20 runs)
#   - Obstacle:   clusters 0-11  (12 runs)
#
# These runs have partial checkpoints. train_loki_task.sh auto-detects
# partial runs (xml_step/ dirs + .pt file, no results.json) and passes
# RESUME_ARGS to continue training from the last iteration.
#
# Uses the same scheduling logic as schedule_remaining_28.sh:
#   Phase 1: Staggered initial launch (one per GPU, 5-min stagger)
#   Phase 2: Sequential scheduling with max-concurrent limit, RAM gating,
#            failure cooldowns, per-job resource profiling
#
# Usage: bash scripts/schedule_resume_32.sh [num_gpus] [max_concurrent_per_gpu] [test_mode]
...
launch_next_job() {
    ...
    local job_desc="task=${task} cluster=${cluster} (GPU=${gpu}) [RESUME]"
...
echo "=== LOKI Targeted Scheduler: 32 Incomplete Evaluations (Resume) ==="
echo "  Locomotion: clusters 0-19  (20 runs)"
echo "  Obstacle:   clusters 0-11  (12 runs)"
...
# Build queue: locomotion 0-19, then obstacle 0-11 (skip any already completed)
for CLUSTER_LABEL in $(seq 0 19); do
    TOTAL_JOBS=$((TOTAL_JOBS + 1))
    if is_completed "locomotion" "$CLUSTER_LABEL"; then
        echo "[SKIP] task=locomotion cluster=${CLUSTER_LABEL} (already completed)"
        SKIPPED=$((SKIPPED + 1))
        continue
    fi
    QUEUE_TASKS+=("locomotion")
    QUEUE_CLUSTERS+=("$CLUSTER_LABEL")
done

for CLUSTER_LABEL in $(seq 0 11); do
    TOTAL_JOBS=$((TOTAL_JOBS + 1))
    if is_completed "obstacle" "$CLUSTER_LABEL"; then
        echo "[SKIP] task=obstacle cluster=${CLUSTER_LABEL} (already completed)"
        SKIPPED=$((SKIPPED + 1))
        continue
    fi
    QUEUE_TASKS+=("obstacle")
    QUEUE_CLUSTERS+=("$CLUSTER_LABEL")
done

PENDING=${#QUEUE_TASKS[@]}
echo ""
echo "Total: $TOTAL_JOBS | Already completed: $SKIPPED | To run (resume): $PENDING"
...
echo ""
echo "=== All 32 incomplete evaluations completed (resumed) ==="
