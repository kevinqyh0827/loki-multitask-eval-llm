...
CLUSTER_LABEL=$3
RNG_SEED=$4
PE_TYPE=${5:-topo}  # "topo" for depth-only, "topo_path" for full path

DROP_FREQ=2
NUM_DROP=2
NUM_SAMPLE=128

LOG_PATH="./log/train_loki_topo"
mkdir -p $LOG_PATH

LOG_FILE="$LOG_PATH/cluster${NUM_CLUSTERS}_idx${CLUSTER_LABEL}_walker${NUM_WALKER}_pe${PE_TYPE}_seed$RNG_SEED.log"

CKPT_PATH="./output/loki/ft/kmeans_cluster/$NUM_CLUSTERS/$CLUSTER_LABEL/walker$NUM_WALKER/pe_${PE_TYPE}/seed$RNG_SEED"
...
PYTHONPATH=./ python tools/train_loki.py \
                        ...
                        ENV.TYPE "ft" \
                        MODEL.TRANSFORMER.POS_EMBEDDING $PE_TYPE \
