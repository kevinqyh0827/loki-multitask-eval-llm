#!/bin/bash
# Targeted scheduler for the 28 remaining NEW evaluations:
#   - Obstacle: clusters 12-19  (8 runs)
#   - Incline:  clusters 0-19  (20 runs)
#
# Uses the same scheduling logic as train_loki_all_cluster_tasks.sh:
...
#
# Usage: bash scripts/schedule_remaining_28.sh [num_gpus] [max_concurrent_per_gpu] [test_mode]
...
echo "=== LOKI Targeted Scheduler: 28 Remaining Evaluations ==="
echo "  Obstacle: clusters 12-19 (8 runs)"
echo "  Incline:  clusters 0-19 (20 runs)"
...
# Build queue: obstacle 12-19, then incline 0-19 (skip any already completed)
for CLUSTER_LABEL in $(seq 12 19); do
...
for CLUSTER_LABEL in $(seq 0 19); do
    TOTAL_JOBS=$((TOTAL_JOBS + 1))
    if is_completed "incline" "$CLUSTER_LABEL"; then
        echo "[SKIP] task=incline cluster=${CLUSTER_LABEL} (already completed)"
        SKIPPED=$((SKIPPED + 1))
        continue
    fi
    QUEUE_TASKS+=("incline")
