...
try:
    ...
    import sys
    sys.path.append("..")
    from metamorph.metamorph.yacs import CfgNode as CN
...
# Unimal100 train
_C.ENV.WALKER_DIR = "../derl/unimals_100/train/"
_C.ENV.JSON_WALKER_DIR = "../derl/unimals_100/train/"
_C.ENV.PKL_WALKER_DIR = "../null"
_C.ENV.WALKER_NAME = '' 
_C.ENV.WALKER_CLUSTER_PATH = ''
_C.ENV.WALKER_CLUSTER = []

# Random xml_debug 200
# _C.ENV.WALKER_DIR = '../derl/xml_debug/ft'
# _C.ENV.PKL_WALKER_DIR = "../derl/xml_debug/ft"
# _C.ENV.JSON_WALKER_DIR = "../null"
# _C.ENV.WALKER_NAME = '0-270005-05-11-53-05' 
...
_C.OBJECT.NUM_BUMPS = 60

# Length of the obstacle box
_C.OBJECT.OBSTACLE_LEN_RANGE = [0.5, 1, 0.1]

_C.OBJECT.BUMP_LEN_RANGE = [0.8, 2.0, 0.2]

# Width of the obstacle box
_C.OBJECT.OBSTACLE_WIDTH_RANGE = [0.5, 1, 0.1]

_C.OBJECT.BUMP_WIDTH_RANGE = [0.8, 2.0, 0.2]

_C.OBJECT.BUMP_HEIGHT_RANGE = [0.05, 0.3, 0.05]
...
_C.PPO.TIMESTEPS = 2560 #2560

# Number of parallel envs for collecting rollouts
_C.PPO.NUM_ENVS = 32 #32
...
_C.PPO.CHECKPOINT_PATH2 = ""
_C.PPO.MODEL_SOUP = False


_C.PPO.NUM_EVAL_EPISODES = 10
...
_C.LOKI.FINETUNE = False

_C.LOKI.RESUME_ITER = -1

_C.LOKI.EVAL = False

_C.LOKI.SEED = 13241322

_C.LOKI.NUM_WALKER = 100

_C.LOKI.UPDATE_WALKER = True

_C.LOKI.NUM_DROP_WALKER = 1

_C.LOKI.CLUSTER_LABEL = 0 # 0 to 49 (total 50 clusters)

_C.LOKI.NUM_CLUSTERS = 50

_C.LOKI.MUTATE_SAMPLE = False

# Max timesteps for evaluation
_C.LOKI.EVAL_TIMESTEPS = 256 #2560

_C.LOKI.WEIGHT_DECAY = 1e-6

_C.LOKI.EPS = 1e-6

_C.LOKI.ADV_CLIP_MAX = 5

_C.LOKI.CLIP_RANGE = 1e-4

_C.LOKI.BATCH_SIZE = 8

_C.LOKI.EVAL_EPISODES = 100

_C.LOKI.NUM_BATCHES = 4

_C.LOKI.NUM_EPOCHS = 100

_C.LOKI.CFG_SCALE = 5.0

_C.LOKI.BITE_SIZE = 8

_C.LOKI.SAMPLE_SIZE = 64

_C.LOKI.UPDATE_BATCH_SIZE = 8

_C.LOKI.RANDOM_AUG = False

_C.LOKI.DROP_FREQ = 1

_C.LOKI.DROP_WARMUP = 10 # warmup before dropping walkers

_C.LOKI.FILTER_CRAWLS = True

_C.LOKI.INIT_DIR = ""

_C.LOKI.MORPH_MIN_LEN = 4

_C.LOKI.MORPH_MAX_LEN = 10

_C.LOKI.NUM_BEST_DROP_WALKER = 8
...
_C.TASK_SAMPLING.AVG_TYPE = "moving_window" # "reward"
...
# Latent morphology dim
_C.MODEL.MORPH_LATENT_DIM = 256
...
_C.MODEL.OBS_TYPES_PRETRAIN = ["proprioceptive", "edges", "obs_padding_mask", "act_padding_mask"]
