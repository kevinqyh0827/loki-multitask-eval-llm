...
try:
    ...
    import sys
    sys.path.append("..")
    import metamorph.metamorph.utils.camera as cu
    import metamorph.metamorph.utils.xml as xu
    from metamorph.metamorph.config import cfg
    from metamorph.metamorph.utils import mjpy as mu
    from metamorph.metamorph.utils import geom as gu
...
class Agent:
    ...
    def get_joint_obs(self, sim):
        ...
        if cfg.ENV.TASK in ["push_box_incline", "manipulation"]:
            ...
            obs["damping"] = sim.model.dof_damping[6:self.agent_qvel_idxs[-1]+1].copy()[:, np.newaxis]
        else:
    ...
    def _get_one_hot_body_idx(self):
        # print(self.agent_body_idxs)
        body_idxs = self.agent_body_idxs
        # print(body_idxs, cfg.MODEL.MAX_LIMBS)
    ...
    def _select_obs(self, obs):
        ...
        if len(obs_to_ret):
            # print("--- ", np.hstack(tuple(obs_to_ret)).shape)
    ...
    def observation_step(self, env, sim):
        ...
        # print(self.combine_limb_joint_obs(limb_obs, joint_obs, env).shape)
        return {
...
if __name__ == '__main__':
    ...
    if os.path.exists(argv):
        for file in os.listdir(argv):
            if file.endswith(".xml"):
                xml = create_agent_xml(os.path.join(argv, file))
                save_file_dir = os.path.join(save_dir, file)
                print(save_file_dir)
