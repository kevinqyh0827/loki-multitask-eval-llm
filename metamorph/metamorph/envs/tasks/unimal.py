...
class UnimalEnv(gym.Env):
    ...
    def __init__(self, xml_str, unimal_id):
        ...
        self.seed()
        self.unimal_xmls, self.unimal_latent_noise = self._load_all_unimals(skip_latent=True)

    def _load_all_unimals(self, skip_xml=False, skip_latent=False):
        dir_path = os.path.join(cfg.ENV.WALKER_DIR, "xml_step", "0") # initial xmls
        xml_paths = [
        ...
        dir_path = os.path.join(cfg.ENV.WALKER_DIR, "latents", "0")
        latent_noise_paths = [
            os.path.join(dir_path, "{}.pt".format(walker_name))
            for walker_name in cfg.ENV.WALKERS
        ]
        if skip_xml:
            return [], [torch.load(path, map_location=torch.device('cpu')).numpy() for path in latent_noise_paths]
        return [create_agent_xml(path) for path in xml_paths], [torch.load(path, map_location=torch.device('cpu')).numpy() for path in latent_noise_paths]

    def _load_unimal_metadata(self, cur_iter):
        # path = os.path.join(cfg.ENV.WALKER_DIR, "metadata", "{}.json".format(self.unimal_id))
        path = os.path.join(
            cfg.ENV.WALKER_DIR, "unimal_pkl_recons_sample", str(cur_iter), "{}.pkl".format(self.unimal_id)
        )
        # return fu.load_json(path)
        return fu.load_pickle(path)
    ...
    def update(self, unimal_id, idx):
        ...
        # )
        self.metadata["mirrored"] = False
        self.xml_str = self.unimal_xmls[idx]
        # self.latent_noise = self.unimal_latent_noise[idx]
    
    def update_xml(self, idx):
        self.xml_str = self.unimal_xmls[idx]

    def update_one_unimal(self, idx=None, new_xml_str=None, cur_iter=0):
        self.unimal_xmls[idx] = new_xml_str
        self.metadata["metadata"] = self._load_unimal_metadata(cur_iter)
        # _, self.unimal_latent_noise = self._load_all_unimals(skip_xml=True)
        # print("Updated unimal xml", idx)
