...
class LOKI:
    ...
    def eval_random_sample(self, sampled_ids, num_samples, initialize=False):
        ...
        # Use DummyVecEnv in batches: avoids fork OOM (no subprocesses) while keeping
        # GPU memory low (only batch_size envs loaded at a time, not all 128).
        ...
        for batch_start in range(0, num_samples, batch_size):
            ...
            env = make_vec_envs(training=False, norm_rew=False, num_env=batch_num,
                                tmp_sample=True, sampled_ids=batch_ids, force_dummy=True)
            set_ob_rms(env, get_ob_rms(self.envs))
    ...
    def eval_random_sample_full(self, sampled_ids, num_samples, initialize=False):
        ...
        for batch_start in range(0, num_samples, batch_size):
            ...
            env = make_vec_envs(training=False, norm_rew=False, num_env=batch_num,
                                tmp_sample_full=True, sampled_ids=batch_ids, force_dummy=True)
            set_ob_rms(env, get_ob_rms(self.envs))
