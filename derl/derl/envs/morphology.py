...
try:
    ...
    from derl.derl.utils import mjpy as mu
...
class SymmetricUnimal:
    ...
    def __init__(self, id_, seed=None, init_path=None, init_pkl=None, init_xml=None):
        self.id = id_

        if init_path:
            self._init_from_state(init_path)
            self.parent_id = fu.path2id(init_path)
        elif init_pkl and init_xml:
            self._init_from_state_string(init_pkl, init_xml)
            self.parent_id = ""
        else:
        ...
        # su.set_seed(seed, use_strong_seeding=False)
        self.curr_mutation = ""
    ...
    def _init_from_state(self, init_path):
        ...
        if "unimal_pkl_recons_sample" in init_path:
            xml_path = init_path.replace("unimal_pkl_recons_sample", "xml_step").replace("pkl", "xml")
        else:
            xml_path = init_path.replace("unimal_init", "xml").replace("pkl", "xml")

        self.root, self.tree = xu.etree_from_xml(xml_path)
        self.worldbody = self.root.findall("./worldbody")[0]
        self.actuator = self.root.findall("./actuator")[0]
        self.contact = self.root.findall("./contact")[0]
    ...
    def _init_from_state_string(self, pkl_str, xml_str):
        self.init_state = fu.load_pickle_from_string(pkl_str)

        self.root, self.tree = xu.etree_from_xml(xml_str, ispath=False)
        self.worldbody = self.root.findall("./worldbody")[0]
        self.actuator = self.root.findall("./actuator")[0]
        self.contact = self.root.findall("./contact")[0]
    ...
    def _construct_head(self):
        ...
        self.limb_metadata[-1]["depth"] = 0
        self.limb_metadata[-1]["limb_density"] = self.body_params["torso_density"]
        self.limb_metadata[-1]["limb_radius"] = head_params["torso_radius"]
    ...
    def mutate(self, op=None):
        if not op:
            ...
            while self.num_limbs >= cfg.LIMB.MAX_LIMBS and op == "grow_limb":
                op = self.choose_mutation()
        else:
    ...
    def _mutate_params_of_limb(self, body, limb_idx, limb_params):
        ...
        self.limb_metadata[limb_idx]["limb_radius"] = limb_params["limb_radius"]
        self.limb_metadata[limb_idx]["limb_height"] = limb_params["limb_height"]
    ...
    def mutate_density(self):
        ...
        for limb_idx,_ in self.limb_metadata.items():
            self.limb_metadata[limb_idx]["limb_density"] = self.body_params["limb_density"]
    ...
    def _mutate_joint_angle(self, limbs, joint_params):
        ...
        for limb_idx in limbs:
            self.limb_metadata[limb_idx]["joint_range"] = {}
            for axis, joint_range in zip(joint_axis, joint_ranges):
                ...
                self.limb_metadata[limb_idx]["joint_range"][axis] = tuple(joint_range)
    ...
    def _mutate_joint_dof(self, limbs, joint_params):
        ...
        for limb_idx in limbs:
            ...
            for insert_pos, axis_ in enumerate(joint_axis):
                ...
                self.limb_metadata[limb_idx]["joint_range"][axis_] = joint_params[
                    "joint_range"
                ][insert_pos]
        ...
        for i in range(50):
            ...
            if is_symmetric and new_contacts is not None:
                ...
                for (l, o, p, a) in zip(limbs, orients, parents, attach_sites):
                    ...
                    parent_limb_idx = xu.name2id(p)
                    if p.get("name") == HEAD:
                        self.limb_metadata[limb_idx]["depth"] = 1
                    else:
                        self.limb_metadata[limb_idx]["depth"] = (
                            self.limb_metadata[parent_limb_idx]["depth"] + 1
                        )
                    self.limb_metadata[limb_idx]["limb_radius"] = lp["limb_radius"]
                    self.limb_metadata[limb_idx]["limb_height"] = lp["limb_height"]
                    self.limb_metadata[limb_idx]["joint_range"] = {}
                    for axis, joint_range in zip(lp["joint_axis"], lp["joint_range"]):
                        self.limb_metadata[limb_idx]["joint_range"][axis] = tuple(joint_range)
                    self.limb_metadata[limb_idx]["limb_density"] = lp["limb_density"]
    ...
    def save_random(self, xml_path, pkl_path):
        self._before_save()
        xml_save_path = os.path.join(xml_path, "{}.xml".format(self.id))
        xu.save_etree_as_xml(self.tree, xml_save_path)
        if self.parent_id:
            mutation_op = self.curr_mutation
        else:
            mutation_op = ""
        ...
        save_path = os.path.join(
            pkl_path, "{}.pkl".format(self.id)
        )
        fu.save_pickle(init_state, save_path)

    def get_pkl_dict(self):
        if self.parent_id:
            mutation_op = self.curr_mutation
        else:
            mutation_op = ""
