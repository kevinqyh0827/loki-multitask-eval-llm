...
class ManhattanReachReward(gym.Wrapper):
    """Reach reward used in MazeNav task."""
    
    def step(self, action):
        obs, rew, done, info = self.env.step(action)
        
        # calculate the Manhattan distance between the agent and the goal
        agent_goal_d_before = np.abs(info["goal_pos"] - info["xy_pos_before"]).sum()

        agent_goal_d_after = np.abs(info["goal_pos"] - info["xy_pos_after"]).sum()
